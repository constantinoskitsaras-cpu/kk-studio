...
export function WorkDetailContent({
  ...
  const nextProject = localizeProject(baseNextProject, locale)
      ...
      <main>
        ...
        <div className="px-6 md:px-10" style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <div className="py-10 md:py-12 grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 border-t border-b border-[#1A1A1A]">
            {[
              { label: t('work.detail.role'),     value: project.role     },
              { label: t('work.detail.software'), value: project.software },
              { label: t('work.detail.client'),   value: project.client   },
            ].map(({ label, value }) => (
              <div key={label}>
                <p
